...
;;
;; Opening the front panel reveals a jumble of wires. Specifically, _two
;; wires_ are connected to a central port and extend outward on a grid.  You
;; trace the path each wire takes as it leaves the central port, one wire per
;; line of text (your puzzle input).
;;
;; The wires _twist and turn_ ((A jumble of twisty little wires, all alike.)),
;; but the two wires occasionally cross paths. To fix the circuit, you need to
;; _find the intersection point closest to the central port_. Because the
;; wires are on a grid, use the Manhattan distance for this measurement. While
...
(module+ test
  (check-equal? (problem-03a "123") 3))

(define (problem-03b input)
  #f)

(module+ test
  (check-equal? (problem-03b "123") 6))
