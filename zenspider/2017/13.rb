...
# across the layer. To succeed, you must not be detected by a scanner.
#
...
# step. Drawing scanners as S, the first few picoseconds look like
# this:
#
# Picosecond 0:
#  0   1   2   3   4   5   6
...
# one layer forward (its first move takes it into layer 0), and then
# the scanners move one step. If there is a scanner at the top of the
# layer as your packet enters it, you are caught. (If a scanner moves
# into the top of its layer while you are there, you are not caught:
# it doesn't have time to notice you before you leave.) If you were to
# do this in the configuration above, marking your current position
# with parentheses, your passage through the firewall would look like
# this:
#
# Initial state:
#  0   1   2   3   4   5   6
...
(0..6).each do |picosec|
  length   = depths[picosec]
  cycle    = 2 * length - 1
  position = picosec % cycle
  # next if picosec != position
  p [picosec, position, cycle, length]
end
