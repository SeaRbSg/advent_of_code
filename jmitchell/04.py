...
def checksum(s):
    ...
    while len(ans) < 5:
        ...
        for k in m_k:
            f.pop(k)
        m_k.sort()
        n = min(len(m_k), 5-len(ans))
        ans += "".join(m_k[:n])
    return ans

def parse_room(code):
    i = code.rfind("-")
    name, rest = code[:i], code[i+1:]
    sector_id, chk = rest[:-7], rest[-6:-1]
    return (name, int(sector_id), checksum(name + sector_id) == chk)

def sector_sum(rooms):
    ans = 0
    for r in rooms:
        _, sect, chk = parse_room(r)
        if chk:
            ans += sect
    return ans

def shift(s, n):
    # print "Shift '%s' by %d" % (s, n)
    l = list(s)
...
def decode_room(r):
    code_name, sect, chk = parse_room(r)
    if not chk:
        return "DECOY"
    name = shift(code_name, sect)
    return "%s-%s" % (name, sect)
...
# print sector_sum(sample_rooms)
print sector_sum(puzzle_input)


# PART TWO
# print decode_room("qzmt-zixmtkozy-ivhz-343[zimth]")
